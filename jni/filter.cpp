...
void QRS_det(double OutputRaw[], int rdIdx,double Output2[], int Output3[],
	...
	double Output1[LENGTH];
	double outRaw[LENGTH];
	double Output[LENGTH];//数组长度4096
	for(int i = 0; i < LENGTH; i++) {

		if((i + rdIdx) >= LENGTH*3) {
			Output1[i] = OutputRaw[i + rdIdx - LENGTH*3];
		}else {
			Output1[i] = OutputRaw[i + rdIdx];
		}
	...
	for(int j = 26; j < LENGTH - 25; j++) {
	         sum += Output1[j+25] - Output1[j-26];
			 Output[j] = sum / 51;
	}
	for(int m = LENGTH -25; m < LENGTH ; m++,n++) {
		         sum += Output1[m] ;
	}
	for(int t = LENGTH -25; t < LENGTH ; t++,n++) {
		         sum -= Output1[t-1];
				 Output[t] = sum / (25 - n);
	}
    ...
    */
	...
	for (int i = 1; i < k; i++) {
		...
		//180很重要~~~结果和MATLAB相近
		if ((IND[i] < m +  100) && (IND[i] > m - 100) && (IND[i] != m))
			//去掉R波附近的极大值点
			IND[i] = 0;
